package org.launchcode;

import java.util.Scanner;

public class AliceSearch {
    public static void main(String[] args){
        String aliceText = "Alice was beginning to get very tired of sitting by her sister " +
                "on the bank, and of having nothing to do: once or twice she had peeped into" +
                " the book her sister was reading, but it had no pictures or conversations" +
                " in it, ‘and what is the use of a book,’ thought Alice ‘without pictures" +
                " or conversation?’";
        Scanner input = new Scanner(System.in);
        System.out.println("String to search for: ");
        String searchString = input.nextLine();
        int index = aliceText.indexOf(searchString);
        System.out.println(index + " " + searchString.length());
        if (aliceText.toLowerCase().contains(searchString.toLowerCase())) {
            System.out.println("Yup it's in there!");
        } else {
            System.out.println("Nope");
        }
        String aliceEdited = aliceText.replaceFirst(searchString, "");
        System.out.println(aliceEdited);
    }
}
